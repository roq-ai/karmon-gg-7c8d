...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    const body = { ...req.body };
    if (body?.discord?.length > 0) {
      const create_discord = body.discord;
      body.discord = {
    ...
    if (body?.minecraft_server?.length > 0) {
      const create_minecraft_server = body.minecraft_server;
      body.minecraft_server = {
        create: create_minecraft_server,
      };
    } else {
      delete body.minecraft_server;
    }
